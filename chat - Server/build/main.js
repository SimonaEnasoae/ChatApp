...
const messages = [new Message(0, 'hey', 'Maria', 'Friends'), new Message(1, 'Hello', 'Maria', 'Colleagues')];
app.use(async (ctx, next) => {
...
let count = 0;
setInterval(() => {
  var users = ['Maria', 'Ana', 'Oana', 'Lucia', 'Dana'];
  var meesagesList = ['Hey', 'Servus', 'Salut', 'Hello', 'Bye', 'See you soon'];
  var id = messages.length;
  var userIndex = Math.floor(Math.random() * users.length);
  var messageIndex = Math.floor(Math.random() * meesagesList.length);
  var message = new Message(id, meesagesList[messageIndex], users[userIndex], `Friends`);
  messages.push(message);
  console.log(`broadcast ${message.text} to Friends`);
  broadcast(message);
  count = ++count % 2;
  id = messages.length;
  userIndex = Math.floor(Math.random() * users.length);
  messageIndex = Math.floor(Math.random() * meesagesList.length);
  message = new Message(id, meesagesList[messageIndex], users[userIndex], `Colleagues`);
  messages.push(message);
  console.log(`broadcast ${message.text} to Colleagues`);
  broadcast(message);
  count = ++count % 2;
}, 5000);
...
module.exports = __webpack_require__(/*! C:\Users\Simona\Desktop\An 3\Model Teo\Mobile\Subiecte\examen chat\src/index.js */"./src/index.js");
